...
class VentaItem(models.Model):
    ...
    def actualizar_stock(self):
        ...
        logger.error(f'las cantidades son estas: cantidad actual: {cantidad_actual} y stock actual: {stock_actual}')
        if stock_actual >= cantidad_actual:
            self.producto.stock = self.producto.stock - cantidad_actual
            logger.error(f'el stock fianl: {self.producto.stock}')
            self.producto.save()
        else:
            logger.error(f"El stock es menor a lo solicitado")
            
        # agregar movimientos al inventario    
